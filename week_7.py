class Bank:
    ...
    #function สำหรับค้นหา instance ของ user ใน user_list
    def search_user_from_id(self,user_ID):
        for that_user in self.__user_list:
            #ถ้า id ที่รับเข้ามาตรงกับ id ของ user ที่อยู่ใน list ให้ส่ง user นั้นออกมา
            if user_ID == that_user.citizen_id:
                return that_user
            
    def search_atm_machine(self,atm_no):
        for that_atm in self.__atm_list:
            #ถ้า num ที่รับเข้ามาตรงกับ num ของ atm ที่อยู่ใน list ให้ส่ง atm นั้นออกมา
            if atm_no == that_atm.atm_no:
                return that_atm
            
    def search_account_from_card(self,card_no):
        for that_user in self.__user_list:
            for that_account in that_user.account_list:
                if card_no == that_account.get_card().card_no:
                    return that_account
                
    def search_account_from_account_no(self, account_no):
        for that_user in self.__user_list:
            for that_account in that_user.account_list:
                if account_no == that_account.account_no:
                    return that_account
                
    def search_seller(self, seller):
        for that_seller in self.__seller_list:
            if seller == that_seller.name:
                return that_seller
...
class User:
    ...
    def search_account(self, account_num):
        for that_account in self.__account_list:
            if account_num == that_account.account_no:
                return that_account
...
class Account:
    ...
    def __add__(self, money):
        self.__amount += money
        self.__transaction.append(Transaction("D", money, self.__amount, None))

    def __sub__(self, money):
        self.__amount -= money
        self.__transaction.append(Transaction("W", money, self.__amount, None))

    def __rshift__(self, input):
        money, target_account = input
        self.__amount -= money
        target_account.__amount += money
        self.__transaction.append(Transaction("T", money, self.__amount, target_account))
        target_account.__transaction.append(Transaction("T", money, self.__amount, None))
    
    def transfer(self, amount, target_account):
        if amount <= 100000 and isinstance(target_account,Account) and amount > 0 and amount <= self.__amount:
...
class SavingAccount(Account):
    ...
    #ที่ init อีกรอบเพราะต้องการเพิ่ม card เข้าไปใน class?
    def __init__(self, account_no, user, amount):
...
class ATM_machine:
    ...
    #โดยการถอนเงินจะต้องไม่เกินอัตราเบิกเงินสูงสุดต่อครั้งและต้องไม่เกินเงินในตู้ ATM และ การโอนเงินต้องไม่เกิน 100,000 บาท
    def withdraw(self, account, amount):
        if amount <= account.amount and amount <= self.withdraw_limit and amount <= self.__money:
            account - amount
            return "Success"
        else:
            return "Error"
    #amount น่าจะแปลว่า จำนวนที่ต้องการจะทำธุรกรรม
    def transfer(self,account, amount, target_account):
        if amount <= 100000 and isinstance(account,Account) and isinstance(target_account,Account) and amount > 0 and amount <= account.amount:
...
class Seller:
    ...
    def search_edc_from_no(self, edc_no):
        for that_edc in self.__edc_list:
            if edc_no == that_edc.edc_no:
                return that_edc
...
class EDC_machine:
    ...
    def paid(self, card, amount, target_account):
        if amount <= 100000 and isinstance(card,Debit_Card) and isinstance(target_account,Account) and amount > 0 and amount <= card.account.amount:
            card.account >> (amount, target_account)
            return "Success"
        else:
            return "Error"
...
# กำหนด รูปแบบของ user ดังนี้ 
     #{รหัสประชาชน :        [ชื่อ,                     ประเภทบัญชี,   หมายเลขบัญชี, จำนวนเงินในบัญชี, ประเภทบัตร, หมายเลขบัตร ]}
...
for all_transaction in hermione_account1.transaction:
